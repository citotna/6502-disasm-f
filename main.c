...
#define P2(_C,_S) case _C:printf(_S,c[2],c[1]);c+=2;break;
#define P1(_C,_S) case _C:printf(_S,     c[1]);c+=1;break;
void print(uint8_t *c,size_t l,uint16_t b) {
    for(uint8_t *e=c+l,*s=c; c<e; ++c) { 
	    ...
	    case 2: printf("%02X %02X   %.3s ",c[1],c[2],ops[opi[*c]]);
	        switch(am[*c]) {P2(2,"$%02X%02X");P2(3,"$%02X%02X,Y");P2(4,"$%02X%02X,X");P2(7,"($%02X%02X)");} break;
	    case 1: printf("%02X      %.3s ",c[1],ops[opi[*c]]); 
	        switch(am[*c]) { case 10: printf("$%04X",(uint16_t)(d+(int8_t)c[1])+2); c++; break;
    		    P1(8,"($%02X,X)");P1(9,"($%02X),Y");P1(5,"#$%02X");P1(11,"$%02X");P1(12,"$%02X,X");P1(13,"$%02X,Y"); } break;
	} putchar('\n');
    } return;
}
